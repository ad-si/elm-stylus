...
nestedRuleContentItem content =
  oneOf
    [ backtrackable <|
        succeed (\a b -> Loop { content | declarations = ( a, b ) :: content.declarations })
        |. symbol (Token "    " GenericProblem)-- 4 spaces for nested rule declarations
        |= property
        |. symbol (Token " " GenericProblem)
        |= value
        |. symbol (Token "\n" GenericProblem)
    , succeed (\nestedRuleExpr -> Loop { content | nestedRules = nestedRuleExpr :: content.nestedRules })
    |. symbol (Token "    " GenericProblem)-- 4 spaces for further nested rules
    |= furtherNestedRule
    , succeed ()
      |> map (\_ -> Done { declarations = List.reverse content.declarations, nestedRules = List.reverse content.nestedRules })
    ]
...
furtherNestedRuleContentItem content =
  oneOf
    [ backtrackable <|
        succeed (\a b -> Loop { content | declarations = ( a, b ) :: content.declarations })
        |. symbol (Token "      " GenericProblem)-- 6 spaces for further nested declarations
        |= property
        |. symbol (Token " " GenericProblem)
        |= value
        |. symbol (Token "\n" GenericProblem)
...
section scts =
  oneOf
    [ succeed
      (\start sct end -> if start == end
          then Done (List.reverse scts)
          else Loop (sct :: scts)
      )
    |= getOffset
    |= oneOf
...
flattenRule parentSelectors content =
  let
    currentRule =
      if List.isEmpty content.declarations
        then []
        else [ Rule ( parentSelectors, { declarations = content.declarations, nestedRules = [] } ) ]
  ...
  case expr of
    Rule ( childSelectors, childContent ) ->
      let
        combinedSelectors =
          List.concatMap
            (\parent -> List.map (\child -> parent ++ " " ++ child) childSelectors
            )
            parentSelectors
      in
