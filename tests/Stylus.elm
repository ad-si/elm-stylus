...
primitiveTests =
  describe
    ...
    , test "Comment" <|
        \_ -> Expect.equal
          (Ok (Comment "Just a test"))
          (run commentLine "// Just a test\n")
    , test "Newlines" <|
        \_ -> Expect.equal (Ok Newlines) (run newlines "\n\n\n")
    , test "No Newlines" <|
        \_ -> Expect.equal
          (Err
              [ { col = 1
...
errorHandlingTests =
  describe
    ...
    [ test "Empty selector fails" <|
        \_ -> Expect.err (run selector "")
    , test "Empty property fails" <|
        \_ -> Expect.err (run (declaration []) "  \n")
    ]
  ...
  describe
    ...
    [ test "Multiple declarations work" <|
        \_ ->
          ...
          case stylusToCss stylusStr of
            Ok css ->
              if String.contains ".test{" css
                then Expect.pass
                else Expect.fail ("Expected .test{ in CSS output: " ++ css)
            Err err ->
